...
export class DetailsSondageComponent implements OnInit {
 ...
 //
  ...
  validedSurvey(id, date){
    ...
    const lien = `<a http://localhost:5000/dashboard>lien</a>`;
    const content  = `La date du sondage auquelle vous avez participe est fixe au: ` + date.date +
     'de '+ date.heure_debut+ ' à '+ date.date.heure_fin + 'cliquer sur' + lien + 'pour plus de details' ;
    console.log(date);
